...
ggsave("prediction_count.pdf", width = 3.38, height = 3.38) 
write.table(results, "prediction_count.tab", row.names=FALSE, sep="\t")
```


Run a statistical analysis of ranking within the results.

```{r}
results = read.table("prediction_count.tab", header = TRUE)
...
sink("prediction_stats_count.tex")
print(xtable(test_frame, digits=c(0, 0, 3, 3, -1, -1), display=c("d", "d", "f", "f", "e", "e")), 
      include.rownames=FALSE)
sink()
```

```{r}
results = read.table("prediction_PSI.tab", header = TRUE)
test_frame = data.frame()
for (nc in c(unique(results$conditions), "all")){
  if(nc == "all"){
    x = results[results$method == "full", "AUC"]
    y = results[results$method == "pairwise", "AUC"] 
  } else {
    x = results[results$conditions == nc & results$method == "full", "AUC"]
    y = results[results$conditions == nc & results$method == "pairwise", "AUC"]  
  }
  wilc = wilcox.test(x, y, paired = TRUE, alternative = "greater")
  tt = t.test(x, y, alternative = "greater")
...
sink("prediction_stats_PSI.tex")
print(xtable(test_frame, digits=c(0, 0, 2, 2, -1, -1), display=c("d", "d", "f", "f", "e", "e")), 
      include.rownames=FALSE)
sink()
```
...
ggsave("prediction_PSI.pdf", width = 3.38, height = 3.38)
